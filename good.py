...
def get_last_price(ticker):
    """1분전 종가 조회"""
    df = pyupbit.get_ohlcv(ticker, interval = "minutes1", count = 5)
...
while True:
    try:
        ...
        print(selected, "60초마다 한번씩 감시중", upbit.get_balance("KRW"), datetime.datetime.now())
        ...
        for select in selected:
            ...
            current_price = get_current_price(select)
            last_price = get_last_price(select)
            if krw > 5000 :
                if macd_1 > signal_1:  
                    if volume > mean:
